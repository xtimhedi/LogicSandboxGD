class_name SaveFlowSerializerArea3D
...
func apply_to_node(node: Node, data: Variant) -> void:
	...
	if payload.has("gravity_space_override"):
		target.gravity_space_override = int(payload["gravity_space_override"]) as Area3D.SpaceOverride
	...
	if payload.has("linear_damp_space_override"):
		target.linear_damp_space_override = int(payload["linear_damp_space_override"]) as Area3D.SpaceOverride
	if payload.has("linear_damp"):
		target.linear_damp = float(payload["linear_damp"])
	if payload.has("angular_damp_space_override"):
		target.angular_damp_space_override = int(payload["angular_damp_space_override"]) as Area3D.SpaceOverride
